...
def load_all() -> None:
    ...
    packages = []
    for package_name in ("providers.captcha", "providers.proxy", "providers.sms", "providers.mailbox"):
        try:
            packages.append(importlib.import_module(package_name))
        except ModuleNotFoundError as exc:
            if exc.name != package_name:
                raise
            logger.debug("Optional provider package missing: %s", package_name)
