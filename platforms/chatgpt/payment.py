...
try:
    ...
    _SUPPORTS_IMPERSONATE = True
except ImportError:
    import requests as cffi_requests  # type: ignore[no-redef]
    _SUPPORTS_IMPERSONATE = False


def _request(method: str, url: str, **kwargs):
    impersonate = kwargs.pop("impersonate", None)
    if _SUPPORTS_IMPERSONATE and impersonate:
        kwargs["impersonate"] = impersonate
    return getattr(cffi_requests, method)(url, **kwargs)
...
def _build_proxies(proxy: Optional[str]) -> Optional[dict]:
    ...
    if proxy_value.startswith("socks5://"):
        proxy_value = "socks5h://" + proxy_value[len("socks5://"):]
...
def fetch_subscription_status_details(account: Account, proxy: Optional[str] = None) -> dict:
    """Return normalized subscription status from the direct usage endpoint only."""
