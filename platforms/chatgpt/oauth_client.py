...
class OAuthClient:
    ...
    def _handle_add_phone_verification(
        ...
        sms_provider_key = str(self.config.get("sms_provider") or "").strip()
        if sms_provider_key and sms_provider_key not in {"smstome", "sms_to_me"}:
            from core.base_sms import create_sms_provider

            provider = create_sms_provider(sms_provider_key, self.config)
            service = str(self.config.get("sms_service") or "dr")
            country = str(self.config.get("sms_country") or "")
            max_attempts = int(self.config.get("phone_retry_limit") or self.config.get("chatgpt_phone_attempts") or 3)
            timeout = int(self.config.get("sms_code_timeout") or 180)
            poll_interval = int(self.config.get("sms_poll_interval") or 5)
            for attempt in range(max(1, max_attempts)):
                activation = None
                try:
                    activation = provider.get_number(service=service, country=country)
                    phone_number = str(getattr(activation, "phone_number", "") or "")
                    activation_id = str(getattr(activation, "activation_id", "") or "")
                    if not phone_number or not activation_id:
                        last_failure = "SMS provider 返回空手机号或 activation"
                        self._log(last_failure)
                        continue
                    self._log(f"步骤5: add_phone 使用 {sms_provider_key} 手机号 {attempt + 1}/{max_attempts}: {phone_number}")
                    sent, next_state, detail = self._send_phone_number(phone_number, device_id, user_agent, sec_ch_ua, impersonate)
                    if not sent or not next_state:
                        last_failure = detail or "add-phone/send 未返回有效状态"
                        self._log(last_failure)
                        mark_failed = getattr(provider, "mark_send_failed", None)
                        if callable(mark_failed):
                            mark_failed(activation_id, last_failure)
                        continue
                    if (
                        next_state.page_type != "phone_otp_verification"
                        ...
                        self._log(last_failure)
                        continue
                    code_result = provider.wait_for_code(activation_id, timeout=timeout, poll_interval=poll_interval)
                    code = str((code_result or {}).get("code") or code_result or "").strip() if not isinstance(code_result, str) else code_result.strip()
                    if not code:
                        resend_ok, resend_detail = self._resend_phone_otp(phone_number, device_id, user_agent, sec_ch_ua, impersonate, next_state)
                        if resend_ok:
                            code_result = provider.wait_for_code(activation_id, timeout=timeout, poll_interval=poll_interval)
                            code = str((code_result or {}).get("code") or code_result or "").strip() if not isinstance(code_result, str) else code_result.strip()
                        if not code:
                            last_failure = resend_detail or f"手机号 {phone_number} 未收到短信验证码"
                            self._log(last_failure)
                            stop_reuse = getattr(provider, "_stop_reuse", None)
                            if callable(stop_reuse):
                                stop_reuse("add_phone sms timeout")
                            continue
                    valid, validated_state, detail = self._validate_phone_otp(code, device_id, user_agent, sec_ch_ua, impersonate, next_state)
                    if valid and validated_state:
                        report_success = getattr(provider, "report_success", None)
                        if callable(report_success):
                            report_success(activation_id)
                        self.config["chatgpt_bound_phone_number"] = phone_number
                        return validated_state
                    last_failure = detail or "手机号 OTP 验证失败"
                    self._log(last_failure)
                except Exception as e:
                    last_failure = f"add_phone SMS provider 异常: {e}"
                    self._log(last_failure)
