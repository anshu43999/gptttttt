...
DEFAULT_AUTH_ACCEPT_LANGUAGE = "ja-JP,ja;q=0.9,en-US;q=0.6,en;q=0.5"
def _open_camoufox_browser(launch_opts: dict):
    from camoufox.sync_api import Camoufox

    return Camoufox(**launch_opts)
    ...
    while time.time() < deadline:
        for selector in selectors:
            try:
                ...
                if locator.count() > 0 and locator.is_visible(timeout=300) and locator.is_enabled(timeout=300):
...
def _click_password_continue_if_available(page, log, *, context: str) -> bool:
    selector = _click_first(page, PASSWORD_CONTINUE_SELECTORS, timeout=1)
    if selector:
        log(f"{context} 已选择使用密码继续: {selector}")
        time.sleep(1)
        return True
    try:
        clicked = bool(
            page.evaluate(
                ...
                () => {
                  ...
                  const target = nodes.find((el) => {
                    ...
                    return visible(el) && /使用密码继续|使用密码登录|continue with password|use password|password/i.test(text);
                  });
...
def _is_email_already_used_error(text: str) -> bool:
...
def _fill_phone_input_like_user(page, selector: str, phone_number: str) -> bool:
    raw = str(phone_number or "").strip()
    expected_digits = "".join(ch for ch in raw if ch.isdigit())
    ...
    try:
        ...
        _browser_pause(page, headed=False)
        try:
            locator.type(raw, delay=random.randint(35, 85))
        except Exception:
            try:
                page.fill(selector, raw)
            except Exception:
                return False
        final_value = str(locator.input_value() or "")
        final_digits = "".join(ch for ch in final_value if ch.isdigit())
        if final_digits and (final_digits.endswith(expected_digits) or expected_digits.endswith(final_digits)):
            return True
    except Exception:
        pass
    ...
    try:
        ok = page.evaluate(
            ...
            ({ selector, value, expectedDigits }) => {
              ...
              return Boolean(digits) && (digits.endsWith(expectedDigits) || expectedDigits.endsWith(digits));
            }
            """,
            {"selector": selector, "value": raw, "expectedDigits": expected_digits},
        )
        return bool(ok)
    except Exception:
        return False
    ...
    for entry in entries:
        ...
        if field == "name":
            ...
            if any(token in hints for token in (" name ", "name", "autocomplete=name", "nombre", "nom", "nome")):
                score += 3
            if any(token in hints for token in ("age", "年龄", "年齢", "何才", "何歳", "edad", "âge", "alter", "idade", "birthday", "birth", "date of birth", "birth year", "year of birth", "出生", "生日", "生年月日", "誕生日", "生年")):
                score -= 8
        elif field == "age":
            if any(token in hints for token in ("age", "年龄", "年齢", "何才", "何歳", "how old", "edad", "âge", "alter", "idade", "나이")):
                score += 10
            if any(token in hints for token in ("full name", "fullname", "全名", "姓名", "氏名", "nombre completo", "nom complet")):
                score -= 10
            if "name" in hints and not any(token in hints for token in ("age", "年龄", "年齢", "何才", "何歳", "edad")):
                score -= 6
            if any(token in hints for token in ("birthday", "birth", "date of birth", "birth year", "year of birth", "出生", "生日", "生年月日", "誕生日", "生年", "fecha de nacimiento", "nascimento")):
                score -= 3
        elif field == "birth_year":
            if any(token in hints for token in ("birth year", "year of birth", "生年")) and not any(token in hints for token in ("生年月", "生年月日", "birthday", "date of birth")):
                score += 12
            if any(token in hints for token in ("full name", "fullname", "全名", "姓名", "氏名", "age", "年齢", "何歳", "生年月", "生年月日", "誕生日")):
                score -= 8
        elif field == "birth_year_month":
            if any(token in hints for token in ("生年月", "birth month", "birth year month", "year and month of birth")) and not any(token in hints for token in ("生年月日", "birthday", "date of birth")):
                score += 12
            if any(token in hints for token in ("full name", "fullname", "全名", "姓名", "氏名", "age", "年齢", "何歳", "生年 ", "誕生日")):
                score -= 8
        else:
            continue
...
def _select_about_you_mode(
    *,
    ...
    has_birth_year_label: bool = False,
    has_birth_year_field: bool = False,
    has_birth_year_month_label: bool = False,
    has_birth_year_month_field: bool = False,
) -> str:
    if has_birth_year_month_field or (has_birth_year_month_label and not has_birthday_field and not has_birthday_select):
        return "birth_year_month"
    if has_birth_year_field or (has_birth_year_label and not has_birthday_field and not has_birthday_select):
        return "birth_year"
    if has_birthday_select:
        return "birthday_select"
    if has_birthday_field and not has_age_field:
        return "birthday"
...
def _derive_registration_state_from_page(page) -> dict:
    ...
    if _is_login_password_url(current_url):
        return _build_manual_flow_state("login_password", current_url)
    state = _extract_flow_state(None, current_url)
            ...
            page.evaluate(
                ...
                () => {
                  ...
                  const hasAgeOrBirth = inputs.some((el) => {
                    const hint = `${el.name || ''} ${el.id || ''} ${el.placeholder || ''}`.toLowerCase();
                    return hint.includes('age') || hint.includes('birth') || hint.includes('birthday') || hint.includes('年龄') || hint.includes('生日') || hint.includes('年齢') || hint.includes('生年月日') || hint.includes('誕生日');
                  });
...
def _is_session_ended_page(page) -> bool:
    text = _get_visible_page_text(page).lower()
    url = str(page.url or "").lower()
    return "your session has ended" in text or ("logged_out" in url and "auth.openai.com" in url)


def _wait_for_signup_entry_transition(page, log, timeout: int = 60) -> dict:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if _click_password_continue_if_available(page, log, context="邮箱页提交后"):
            time.sleep(0.5)
            continue
        ...
        if _is_session_ended_page(page):
            return _build_manual_flow_state("session_ended", str(page.url or ""))
        ...
        time.sleep(0.25)
    try:
        _dump_debug(page, "signup_entry_stalled")
    except Exception:
        pass
...
def _start_browser_signup_via_page(page, email: str, log) -> dict:
    for entry_url in (f"{CHATGPT_APP}/auth/signup", f"{OPENAI_AUTH}/create-account", PLATFORM_LOGIN_ENTRY, f"{OPENAI_AUTH}/log-in"):
        try:
        ...
        try:
            page.wait_for_function(
                "([selector, value]) => { const el = document.querySelector(selector); return el && String(el.value || '').trim().toLowerCase() === String(value || '').trim().toLowerCase(); }",
                [email_selector, email],
                timeout=3000,
            )
        except Exception:
            if not _fill_input_like_user(page, email_selector, email):
                raise RuntimeError("邮箱页填写后未保持输入值")
        ...
        transition = _wait_for_signup_entry_transition(page, log)
        if transition.get("page_type") == "session_ended":
            log("邮箱页提交后进入 session ended 页面，切换下一个注册入口重试")
            continue
        return transition
...
def _start_browser_signup_via_authorize(page, email: str, device_id: str, log) -> dict:
    log("访问 ChatGPT 首页...")
    try:
        page.goto(f"{CHATGPT_APP}/", wait_until="domcontentloaded", timeout=120000)
    except Exception as exc:
        log(f"ChatGPT 首页访问超时，继续使用 request 级 CSRF/signin fallback: {str(exc).splitlines()[0][:160]}")
    ...
    log(f"提交邮箱: {email}")
    authorize_url = _start_browser_signin(page, email, device_id, csrf_token, screen_hint="signup")
    if not authorize_url:
        try:
            detail = str(page.evaluate("() => window.__lastSigninOpenAiText || ''") or "")
        except Exception:
            detail = ""
        raise RuntimeError(f"提交邮箱失败，未获取 authorize URL: {detail[:300]}")
...
def _dump_debug(page, prefix: str) -> None:
    ...
    try:
        page.screenshot(path=f"{tmpdir}/{prefix}.png")
    except Exception:
        pass
    try:
        with open(f"{tmpdir}/{prefix}.html", "w", encoding="utf-8") as f:
...
def _build_browser_headers(
    *,
    ...
    headers = {
        ...
        "accept-language": DEFAULT_AUTH_ACCEPT_LANGUAGE,
...
def _infer_page_type(data: dict | None, current_url: str = "") -> str:
    ...
    if "/api/oauth/oauth2/auth" in url:
        return "external_url"
    if host.endswith("chatgpt.com") and path in {"/auth/login", "/auth/signup"}:
        return "login_email"
...
def _is_oauth_callback_url(url: str) -> bool:
    ...
    if "code=" not in value and "error=" not in value:
        return False
    return "/auth/callback" in value or "localhost" in value
...
def _browser_fetch(page, url: str, *, method: str = "GET", headers: dict | None = None, body: str | None = None, redirect: str = "manual", timeout_ms: int = 30000) -> dict:
    script = """
        ...
        """
    payload = {"url": url, "method": method, "headers": headers or {}, "body": body, "redirect": redirect, "timeoutMs": timeout_ms}
    last_error = ""
    for _ in range(2):
        try:
            ...
            last_error = str(exc)
            if "Execution context was destroyed" not in last_error and "navigation" not in last_error.lower():
                break
            try:
                page.wait_for_load_state("domcontentloaded", timeout=5000)
            except Exception:
                time.sleep(1)
    return {"ok": False, "status": 0, "url": url, "headers": {}, "text": last_error, "data": None}
...
def _follow_redirects_for_code(session, start_url: str, log, *, max_redirects: int = 12) -> str:
    ...
    for idx in range(max_redirects):
        ...
        if _is_oauth_callback_url(next_url):
            return next_url
...
def _complete_oauth_with_session(cookies_dict: dict, oauth_start, proxy: str | None, log, callback_handler=None) -> dict | None:
    ...
    try:
        ...
        log(f"  curl_cffi 不可用，使用 requests 会话补全 OAuth: {exc}")
        s = cffi_requests.Session()
        s.headers.update({"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36"})
    if proxy:
    ...
    try:
        ...
        direct_callback = _is_oauth_callback_url(next_url)
        if direct_callback:
            if callback_handler:
                return callback_handler(next_url, oauth_start, proxy)
        ...
        if not callback_url:
            ...
            return None
        if callback_handler:
            return callback_handler(callback_url, oauth_start, proxy)
        result_json = submit_callback_url(
...
def _submit_callback_result(callback_url: str, oauth_start, proxy: str | None, callback_handler=None) -> dict:
    if callback_handler:
        return callback_handler(callback_url, oauth_start, proxy)
    from .oauth import submit_callback_url
...
def _extract_callback_url_from_exception(exc: Exception) -> str:
    ...
    return callback_url if _is_oauth_callback_url(callback_url) else ""


def _derive_oauth_state_from_page(page) -> dict:
    state = _derive_registration_state_from_page(page)
    page_type = str(state.get("page_type") or "")
    current_url = str(state.get("current_url") or page.url or "")
    if page_type and page_type != "chatgpt_home":
        return state
...
def _submit_login_phone_via_page(page, phone_number: str, log) -> dict:
    clicked = _click_first(page, PHONE_LOGIN_SELECTORS, timeout=5)
    if clicked:
        log(f"OAuth 登录页已切换手机号登录: {clicked}")
        try:
            page.wait_for_load_state("domcontentloaded", timeout=8000)
        except Exception:
            pass
        time.sleep(2)
    input_selector = _wait_for_any_selector(page, PHONE_INPUT_SELECTORS, timeout=25)
    if not input_selector:
        try:
            debug_dir = Path("output") / "debug"
            debug_dir.mkdir(parents=True, exist_ok=True)
            screenshot_path = debug_dir / f"phone_login_missing_{int(time.time())}.png"
            page.screenshot(path=str(screenshot_path), full_page=True)
            log(f"OAuth 手机号登录页未找到输入框，截图: {screenshot_path}")
        except Exception:
            pass
        raise RuntimeError("OAuth 手机号登录页未找到手机号输入框")
    if not _fill_phone_input_like_user(page, input_selector, phone_number):
        raise RuntimeError("OAuth 手机号登录页填写失败")
    log(f"OAuth 手机号登录输入框: {input_selector}")
    _browser_pause(page)
    submit_selector = _click_first(page, EMAIL_SUBMIT_SELECTORS, timeout=8)
    if submit_selector:
        log(f"OAuth 手机号登录已点击继续按钮: {submit_selector}")
    elif _submit_form_with_fallback(page, input_selector):
        log("OAuth 手机号登录使用表单 fallback 提交")
    else:
        raise RuntimeError("OAuth 手机号登录未找到 Continue 按钮")
    deadline = time.time() + 20
    ...
    while time.time() < deadline:
        ...
        if page_type == "email_otp_verification" and "email-verification" not in current_url_l and "email-otp" not in current_url_l:
            page_type = ""
        if page_type in {
            "login_password",
            "create_account_password",
        ...
        if error_text:
            return {"ok": False, "status": 400, "url": current_url, "data": None, "text": error_text}
        time.sleep(0.5)
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "OAuth 手机号提交后未跳转"}



def _wait_oauth_page_settled(page, log, *, timeout: float = 12.0, stable_seconds: float = 1.5) -> bool:
    """Wait until the auth page is not visibly mid-load before clicking controls."""
    ...
    while time.time() < deadline:
        ...
        try:
            snapshot = page.evaluate(
                ...
                () => {
                  ...
                  const buttons = Array.from(document.querySelectorAll('button, a, [role="button"], input[type="radio"]'))
                    .filter((el) => {
                      const r = el.getBoundingClientRect();
        ...
        signature = "|".join([
            ...
            str(snapshot.get("textLength") or ""),
            str(snapshot.get("buttons") or ""),
        ])
        now = time.time()
        if ready_state == "complete" and signature and signature == last_signature:
            if not stable_since:
                stable_since = now
        ...
        time.sleep(0.4)
    log("  OAuth 页面仍在加载或 DOM 未稳定，暂不点击账号选择")
    return False


def _click_choose_account_via_page(page, log) -> bool:
    selectors = [
        ...
        'button:has-text("選ぶ")',
        '[role="button"]:has-text("選ぶ")',
        'button:has-text("続行")',
        '[role="button"]:has-text("続行")',
        'button:has-text("选择")',
        '[role="button"]:has-text("选择")',
        'button:has-text("選擇")',
        '[role="button"]:has-text("選擇")',
    ]
    def _activate_locator(locator, text: str) -> bool:
        try:
    ...
    for selector in selectors:
        try:
            ...
            if locator.count() and locator.is_visible(timeout=700):
                text = (locator.inner_text(timeout=700) or selector).strip()
                if _activate_locator(locator, text):
                    return True
        except Exception:
            continue
    ...
    clicked = page.evaluate("""
        () => {
          ...
          const actionable = Array.from(document.querySelectorAll('button, a, [role="button"], input[type="radio"], [tabindex], li, div'))
            .filter(visible)
            .map((el) => ({ el, text: String(el.innerText || el.textContent || el.getAttribute('aria-label') || '').replace(/\s+/g, ' ').trim() }))
            .filter(x => x.text && x.text.length < 220);
          const positive = /selecionar|select|continue|choose|選択|選ぶ|続行|次へ|选择|選擇|继续/i;
          const negative = /use another|different|outra|outro|其他|別の|他の|back|戻る|返回|sign out|ログアウト|退出/i;
          const accountButton = actionable.find(x => positive.test(x.text) && !negative.test(x.text));
          const accountRow = actionable.find(x => !negative.test(x.text) && /@|\+\d{6,}|auth0|account|アカウント|账号|帳號/i.test(x.text));
          const radio = Array.from(document.querySelectorAll('input[type="radio"]')).find(visible);
          const target = accountButton?.el || accountRow?.el || radio;
          if (!target) return '';
          ...
          return accountButton?.text || accountRow?.text || 'radio';
        }
    """)
    if clicked:
        log(f"  OAuth 已选择已有账号: {str(clicked)[:80]}")
        time.sleep(1.5)
        return True
    return False

def _do_codex_oauth(page, cookies_dict: dict, email: str, password: str, otp_callback, phone_callback, proxy: str | None, log, bind_email: str | None = None, redirect_uri: str | None = None, client_id: str | None = None, authorize_url: str | None = None, callback_handler=None, otp_baseline_callback=None) -> dict | None:
    """在真实浏览器会话内完成 Codex OAuth，返回 token 包或外部回调提交结果。"""
    from .oauth import OAuthStart, generate_oauth_url
    ...
    if authorize_url:
        ...
        for part in parsed.query.split("&"):
            if part.startswith("state="):
                ...
                break
        oauth_start = OAuthStart(auth_url=str(authorize_url), state=state, code_verifier="", redirect_uri=redirect_uri or CODEX_REDIRECT_URI, client_id=client_id or CODEX_CLIENT_ID)
    else:
    ...
    try:
        try:
            ...
            if callback_url:
                ...
                return _submit_callback_result(callback_url, oauth_start, proxy, callback_handler)
            current_after_timeout = str(page.url or "")
            if "auth.openai.com" in current_after_timeout or "chatgpt.com" in current_after_timeout:
                log(f"  OAuth bootstrap 页面加载超时但已导航，继续状态机: {current_after_timeout[:120]}")
            else:
                raise
        ...
        last_choose_account_click = 0.0
        email_otp_retry_count = 0
        for step in range(60):
            state = _derive_oauth_state_from_page(page)
            current_url = str(page.url or "")
            next_url = str(state.get("continue_url") or "").strip()
            log(
                f"  OAuth state step[{step+1}/60]: "
                f"page={state.get('page_type') or '-'} next={next_url[:60]}"
                f" url={current_url[:120]}"
            )

            callback_url = ""
            if _is_oauth_callback_url(current_url):
                callback_url = current_url
            elif _is_oauth_callback_url(next_url):
                callback_url = next_url
            if callback_url:
                return _submit_callback_result(callback_url, oauth_start, proxy, callback_handler)
            ...
            if (
                ...
                continue
            if state["page_type"] == "choose_account":
                now = time.time()
                if now - last_choose_account_click >= 12:
                    log("  OAuth 页面需要选择已有账号，先等待页面加载稳定...")
                    if not _wait_oauth_page_settled(page, log, timeout=12, stable_seconds=1.2):
                        time.sleep(2)
                        continue
                    if _click_choose_account_via_page(page, log):
                        last_choose_account_click = time.time()
                        choose_account_clicks += 1
                        deadline = time.time() + 10
                        while time.time() < deadline:
                            time.sleep(0.5)
                            post_click_url = str(page.url or "")
                            if "choose-an-account" not in post_click_url:
                                log(f"  OAuth 账号选择后跳转: {post_click_url[:120]}")
                                break
                        if choose_account_clicks >= 3 and "choose-an-account" in str(page.url or ""):
                            raise RuntimeError("OAuth choose_account stuck after repeated clicks; likely proxy/sentinel/browser navigation failure")
                        continue
                else:
                    log("  OAuth 已点击账号选择，等待 auth.openai.com 跳转...")
                    time.sleep(2)
                    continue

            if state["page_type"] == "add_email":
                bind_target = str(bind_email or email or "").strip()
                log(f"  OAuth 页面需要绑定邮箱，提交: {bind_target}")
                email_resp = _submit_login_email_via_page(page, bind_target, log)
                log(f"  OAuth 绑定邮箱提交状态: {email_resp.get('status', 0)}")
                if not email_resp.get("ok"):
                    error_text = str(email_resp.get("text") or "")
                    if _is_email_already_used_error(error_text):
                        return {"error": "EMAIL_ALREADY_USED", "email": bind_target, "text": error_text}
                    raise RuntimeError(f"OAuth 绑定邮箱提交失败: {error_text[:300]}")
                if callable(otp_baseline_callback):
                    try:
                        otp_baseline_callback()
                    except Exception as exc:
                        log(f"  OAuth 邮箱验证码基线重置失败: {str(exc)[:160]}")
                continue

            if state["page_type"] == "login_email":
                if str(email or "").strip().startswith("+"):
                    log("  OAuth 页面需要手机号登录，提交手机号...")
                    email_resp = _submit_login_phone_via_page(page, email, log)
                    log(f"  OAuth 手机号页提交状态: {email_resp.get('status', 0)}")
                    if not email_resp.get("ok"):
                        raise RuntimeError(f"OAuth 手机号页提交失败: {(email_resp.get('text') or '')[:300]}")
                else:
            ...
            if state["page_type"] == "email_otp_verification":
                ...
                if not otp_resp.get("ok"):
                    error_text = str(otp_resp.get("text") or "")
                    ...
                    if wrong_code and email_otp_retry_count < 3:
                        email_otp_retry_count += 1
                        log(f"  OAuth 邮箱验证码错误，重新发送并等待新验证码: retry={email_otp_retry_count}")
                        ...
                        log(f"  OAuth 邮箱验证码重新发送按钮: {clicked or 'not_found'}")
                        if callable(otp_baseline_callback):
                            try:
                                otp_baseline_callback()
                            except Exception as exc:
                        ...
                        time.sleep(2)
                        continue
                    raise RuntimeError(f"OAuth 验证码校验失败: {error_text[:300]}")
                email_otp_retry_count = 0
                continue
            ...
            if state["page_type"] in {"consent", "workspace_selection", "organization_selection", "external_url"}:
                browser_result = _complete_oauth_in_browser(page, oauth_start, proxy, log, callback_handler=callback_handler)
                if browser_result:
                    return browser_result
                cookies_dict = _get_cookies(page)
                session_result = _complete_oauth_with_session(cookies_dict, oauth_start, proxy, log, callback_handler=callback_handler)
                if session_result:
    ...
    cookies_dict = _get_cookies(page)
    result = _complete_oauth_with_session(cookies_dict, oauth_start, proxy, log, callback_handler=callback_handler)
    if result:
        return result
...
def _complete_oauth_callback_navigation(page, state: dict, log) -> None:
    page_type = str(state.get("page_type") or "")
    callback_url = str(state.get("current_url") or state.get("continue_url") or "")
    if page_type not in {"callback", "oauth_callback"} and not ("/api/auth/callback/openai" in callback_url and "code=" in callback_url):
        return
    if not callback_url:
        return
    safe = urlparse(callback_url)
    log(f"等待 OAuth callback 落地: {safe.scheme}://{safe.netloc}{safe.path}")
    try:
        if str(page.url or "") == callback_url:
            page.wait_for_load_state("domcontentloaded", timeout=60000)
        elif hasattr(page, "context") and hasattr(page.context, "request"):
            try:
                response = page.context.request.get(callback_url, timeout=60000)
                log(f"OAuth callback context 请求状态: {getattr(response, 'status', 0)}")
                page.goto(f"{CHATGPT_APP}/", wait_until="domcontentloaded", timeout=60000)
            except Exception as exc:
                log(f"OAuth callback context 请求失败: {str(exc).splitlines()[0][:180]}")
        try:
            page.wait_for_url(lambda url: "chatgpt.com" in str(url) and "/auth/" not in str(url), timeout=60000)
        except Exception:
            pass
        token = _wait_for_access_token(page, timeout=45)
        if token:
            log("OAuth callback 已落地并可读取 session access_token")
        else:
            log("OAuth callback 已跳转，但 session access_token 暂未可读")
    except Exception as exc:
        log(f"OAuth callback 落地等待失败: {str(exc).splitlines()[0][:180]}")
...
def _is_registration_complete(state: dict) -> bool:
    ...
    if page_type in {"callback", "oauth_callback"}:
        return True
    if page_type == "chatgpt_home" and "/auth/" not in url:
        return True
    return "chatgpt.com" in url and "/auth/" not in url and "redirect_uri" not in url and "about-you" not in url
...
def _is_phone_recently_or_already_used_error(text: str) -> bool:
    value = str(text or "").lower()
    markers = (
        "phone_number_in_use",
...
def _is_phone_otp_verification_state(state: dict) -> bool:
    target = f"{state.get('continue_url') or ''} {state.get('current_url') or ''}".lower()
    return str(state.get("page_type") or "") == "phone_otp_verification" or "phone-verification" in target


def _extract_add_phone_api_detail(result: dict) -> str:
    ...
    if isinstance(data, (dict, list)):
        try:
            ...
            return str(data)
    return str((result or {}).get("status") or "").strip()


def _classify_add_phone_api_issue(result: dict) -> str:
    status = int((result or {}).get("status") or 0)
    detail = _extract_add_phone_api_detail(result).lower()
    if status == 403 or any(marker in detail for marker in ("cloudflare", "cf_chl", "turnstile", "captcha")):
        return "cloudflare"
    if status in (404, 405, 410) or "route error" in detail or "not found" in detail:
        return "route_error"
    if status == 0 or status >= 500 or any(marker in detail for marker in (
        "failed to fetch",
        "networkerror",
        ...
        result = page.evaluate(
            ...
            () => {
              ...
              const fieldError = (form) => {
                if (!form) return '';
                for (const selector of ['.react-aria-FieldError', '[slot="errorMessage"]', '[id$="-error"]', '[role="alert"]', '[class*="error" i]']) {
              ...
              return {
                ...
                addPhoneError: fieldError(addPhoneForm),
                verifyError: fieldError(verificationForm),
              };
            }
            """
        )
...
def _wait_for_phone_verification_ready(page, *, timeout: int = 30) -> dict:
    deadline = time.time() + timeout
    last = {}
    while time.time() < deadline:
        last = _phone_page_status(page)
        if last.get("phoneVerificationReady") or last.get("addPhoneError"):
            return last
        time.sleep(0.25)
    return last


def _submit_add_phone_dom(page, *, phone_number: str, dial_code: str, local_number: str, country_name: str, log) -> dict:
    e164 = "+" + str(phone_number or "").lstrip("+").strip()
    national = str(local_number or "").strip() or e164
    payload = {"phoneNumber": e164, "nationalPhoneNumber": national}
    try:
        result = page.evaluate(
            """
            async (payload) => {
              ...
              const sms = radios.find((input) => /sms/i.test(String(input.value || input.closest('label')?.textContent || '')));
              if (sms) {
                (sms.closest('label') || sms).click();
                sms.checked = true;
                sms.dispatchEvent(new Event('input', { bubbles: true }));
                sms.dispatchEvent(new Event('change', { bubbles: true }));
                if (channelInput) setNativeValue(channelInput, 'sms');
                await sleep(120);
              }
              const phoneInput = form.querySelector('input[type="tel"], input[name="__reservedForPhoneNumberInput_tel"], input[autocomplete="tel"], input[autocomplete="tel-national"], input[name="phoneNumber"], input[name="phone_number"]');
              ...
              const hidden = form.querySelector('input[name="phoneNumber"], input[name="phone_number"], input[name="phone"]');
              if (hidden && hidden !== phoneInput) setNativeValue(hidden, payload.phoneNumber);
              await sleep(260);
              const buttons = Array.from(form.querySelectorAll('button[type="submit"], input[type="submit"], button'));
              const submit = buttons.find((button) => visible(button) && !button.disabled && button.getAttribute('aria-disabled') !== 'true') || buttons.find((button) => visible(button));
              if (!submit) return { ok: false, reason: 'missing_submit_button', url: location.href, visibleValue: phoneInput.value || '', hiddenValue: hidden ? hidden.value : '' };
              if (submit.disabled || submit.getAttribute('aria-disabled') === 'true') {
                return { ok: false, reason: 'submit_disabled_after_fill', url: location.href, visibleValue: phoneInput.value || '', hiddenValue: hidden ? hidden.value : '' };
              }
              submit.click();
              return { ok: true, url: location.href, channel: channelInput ? channelInput.value : (sms ? 'sms' : ''), visibleValue: phoneInput.value || '', hiddenValue: hidden ? hidden.value : '' };
            }
            """,
            payload,
        )
    ...
    if result.get("ok"):
        log(f"  add-phone DOM 提交: channel={result.get('channel') or '-'} visible={'yes' if result.get('visibleValue') else 'no'} hidden={'yes' if result.get('hiddenValue') else 'no'}")
    return result


def _is_phone_required_error(text: str) -> bool:
...
def _submit_add_phone_via_trusted_ui(page, *, phone_number: str, local_number: str, country_selected: bool, log) -> str:
    ...
    filled = _fill_phone_input_like_user(page, phone_input_sel, fill_value)
    if not filled and fill_value != phone_number:
        log("  本地号码填写失败，尝试完整 E.164 号码...")
        filled = _fill_phone_input_like_user(page, phone_input_sel, phone_number)
        if filled:
    ...
    if send_sel:
        ...
        return send_sel
    if _submit_form_with_fallback(page, phone_input_sel):
        log("  未找到发送按钮，已使用表单 fallback 提交")
        return "form-fallback"
    raise RuntimeError("未找到发送验证码按钮")


def _force_submit_add_phone_form(page, log) -> str:
    """Retry add-phone submit with real pointer/form events after DOM value injection."""
    selectors = [
        'form[action*="/add-phone" i] button[type="submit"]',
        'form[action*="/add-phone" i] input[type="submit"]',
        'button:has-text("Send code via SMS")',
        'button:has-text("Send code")',
        'button:has-text("Send via SMS")',
        'button:has-text("Send")',
    ...
    for selector in selectors:
        try:
            ...
            try:
                ...
                log(f"  add-phone 二次点击提交按钮: {selector}")
                return selector
            except Exception:
                try:
                    box = target.bounding_box(timeout=1000)
                    if box:
                        page.mouse.move(box["x"] + box["width"] * 0.5, box["y"] + box["height"] * 0.5, steps=8)
                        page.mouse.down()
                        time.sleep(0.08)
                        page.mouse.up()
                        log(f"  add-phone 二次鼠标提交按钮: {selector}")
                        return selector
                except Exception:
                    pass
        except Exception:
    ...
    try:
        ...
        if submitted:
            log("  add-phone 二次 requestSubmit 提交表单")
...
def _wait_add_phone_after_submit(page, log, *, timeout: int = 30) -> dict:
    status = _wait_for_phone_verification_ready(page, timeout=timeout)
    if status.get("phoneVerificationReady") or status.get("addPhoneError"):
        return status
    retry_selector = _force_submit_add_phone_form(page, log)
    if retry_selector:
        status = _wait_for_phone_verification_ready(page, timeout=20)
        status["submit_retry"] = retry_selector
    return status
...
def _submit_phone_otp_dom(page, code: str, log) -> dict:
    ...
    if not isinstance(result, dict) or not result.get("ok"):
        return {"ok": False, "status": 0, "url": str((result or {}).get("url") or page.url), "text": str((result or {}).get("reason") or "phone otp dom submit failed")}
...
def _handle_add_phone_challenge(
    ...
    *,
    ...
    """在 add-phone 页面优先走 browser-context API，失败后回退 UI 完成手机号验证。"""
    ...
    for phone_attempt in range(max_phone_attempts):
        ...
        try:
            ...
            should_retry = (
                ...
                or _is_phone_recently_or_already_used_error(error_msg)
                or "無効" in error_msg
                or "invalid" in lowered
            )
            if not should_retry:
                raise
            log(f"⚠️ 当前绑定手机号不可用，已标记并准备换号重试: {error_msg[:120]}")
...
def _do_add_phone_attempt(
    ...
    *,
    ...
    current_url = str(page.url or "")
    if "add-phone" not in current_url and "phone-verification" not in current_url:
    ...
    if not submit_result.get("ok"):
        ...
        _browser_pause(page)
        _submit_add_phone_via_trusted_ui(
            page,
            phone_number=phone_number,
            local_number=local_number,
        ...
        used_trusted_ui = True

    phone_status = _wait_add_phone_after_submit(page, log, timeout=30)
    if phone_status.get("addPhoneError"):
        error_text = str(phone_status.get("addPhoneError") or "")
        if _is_phone_required_error(error_text) and not used_trusted_ui:
            log(f"  add-phone 首次提交后仍提示缺少手机号，改用真实键鼠路径重填重点: {error_text[:120]}")
            country_selected = _select_phone_country_ui(page, dial_code, country_name, log)
            _browser_pause(page)
            _submit_add_phone_via_trusted_ui(
                page,
    ...
    if not phone_status.get("phoneVerificationReady"):
        ...
        if error_text:
            if _is_phone_required_error(error_text) and not used_trusted_ui:
                log(f"  add-phone 未进入验证码页且提示缺少手机号，改用真实键鼠路径重填重点: {error_text[:120]}")
                country_selected = _select_phone_country_ui(page, dial_code, country_name, log)
                _browser_pause(page)
                _submit_add_phone_via_trusted_ui(
                    page,
                ...
                if phone_status.get("phoneVerificationReady"):
                    ...
                    error_text = str(phone_status.get("addPhoneError") or _extract_auth_error_text(page) or error_text)
            if error_text:
                if hasattr(phone_callback, "mark_send_failed"):
                    phone_callback.mark_send_failed(error_text)
                raise RuntimeError(f"手机号提交失败: {error_text[:200]}")
    ...
    log(f"手机号提交成功({send_path.upper()})，开始等待短信验证码...")
    ...
    for code_attempt in range(3):
        ...
        if not otp_resp.get("ok") and "missing_phone_verification" in str(otp_resp.get("text") or ""):
            otp_resp = _submit_otp_via_page(page, sms_code, log, device_id=device_id, user_agent=user_agent)
        ...
        failure_detail = page_error or _extract_add_phone_api_detail(otp_resp)
        if _is_phone_recently_or_already_used_error(failure_detail):
            log(f"手机号被 OpenAI 判定近期使用/达到绑定限制，标记并换号: {failure_detail[:120]}")
            if hasattr(phone_callback, "mark_send_failed"):
                phone_callback.mark_send_failed(failure_detail or "phone recently used")
            raise RuntimeError(f"手机号已被使用: {failure_detail[:200] if failure_detail else f'status {otp_status}'}")
        if _is_invalid_phone_otp_response(otp_resp) or (
            failure_detail and any(kw in failure_detail.lower() for kw in ("invalid", "incorrect", "wrong", "expired"))
        ):
            log(f"短信验证码被判定无效: {failure_detail[:100]}，继续等待下一条...")
            if hasattr(phone_callback, "mark_code_failed"):
                phone_callback.mark_code_failed(failure_detail or "invalid otp code")
            continue
        if hasattr(phone_callback, "mark_code_failed"):
            phone_callback.mark_code_failed(failure_detail or f"status {otp_status}")
        raise RuntimeError(f"短信验证码校验失败: {failure_detail[:200] if failure_detail else f'status {otp_status}'}")
...
def _get_browser_csrf_token(page) -> str:
    ...
    try:
        response = page.context.request.get(
            f"{CHATGPT_APP}/api/auth/csrf",
            headers={"accept": "application/json", "referer": f"{CHATGPT_APP}/auth/login"},
            timeout=30000,
        )
        if response.ok:
            data = response.json()
...
def _start_browser_signin(page, email: str, device_id: str, csrf_token: str, screen_hint: str = "login_or_signup") -> str:
    ...
    variants = [
        {**base_query, "auth_session_logging_id": str(uuid.uuid4()), "screen_hint": str(screen_hint or "signup"), "login_hint": email},
        base_query,
    ]
    body = urlencode(
        {
    ...
    for query_params in variants:
        ...
        if result.get("ok") and isinstance(result.get("data"), dict):
            ...
            if url:
                return url
        last_text = str(result.get("text") or result.get("data") or "")[:300]
    for query_params in variants:
        query = urlencode(query_params)
        try:
            ...
            last_text = str(exc)[:300]
    try:
        page.evaluate("(text) => { window.__lastSigninOpenAiText = text; }", last_text)
    except Exception:
        pass
    return ""
...
def _browser_authorize(page, auth_url: str, log) -> str:
    ...
    try:
        ...
        current_url = str(page.url or "")
        if "auth.openai.com/email-verification" in current_url or "auth.openai.com/create-account" in current_url:
            log(f"Authorize 等待超时但页面已进入: {current_url[:120]}")
            return current_url
...
def _send_browser_add_phone_number(page, phone_number: str, device_id: str, user_agent: str, referer: str) -> dict:
    headers = _build_browser_headers(
        user_agent=user_agent,
        accept="application/json",
        referer=referer or f"{OPENAI_AUTH}/add-phone",
        origin=OPENAI_AUTH,
        content_type="application/json",
        extra_headers={
            "sec-fetch-site": "same-origin",
...
def _validate_browser_phone_otp(page, code: str, device_id: str, user_agent: str, referer: str) -> dict:
    headers = _build_browser_headers(
        user_agent=user_agent,
        accept="application/json",
        referer=referer or f"{OPENAI_AUTH}/phone-verification",
        origin=OPENAI_AUTH,
        content_type="application/json",
        extra_headers={
            "sec-fetch-site": "same-origin",
    ...
    sentinel = _build_browser_sentinel_token(page, device_id, "phone_otp_validate", user_agent)
    if sentinel:
        headers["openai-sentinel-token"] = sentinel
    _browser_pause(page)
    return _browser_fetch(
...
def _complete_oauth_in_browser(page, oauth_start, proxy, log, callback_handler=None) -> dict | None:
    ...
    def _try_extract_callback(url: str) -> dict | None:
        if not _is_oauth_callback_url(url):
            return None
        if callback_handler:
            return callback_handler(url, oauth_start, proxy)
        try:
            ...
            if "state" in str(ve) and "localhost" in url and "code=" in url:
                try:
                    ...
                    if code:
                        from .oauth import exchange_code_for_token_record
                        return exchange_code_for_token_record(
                            code=code,
                            code_verifier=oauth_start.code_verifier,
                            redirect_uri=oauth_start.redirect_uri,
                            client_id=oauth_start.client_id,
                    ...
                    log(f"  callback code 换 token 失败: {exc}")
            return None
        except Exception as exc:
            log(f"  callback URL 处理失败: {exc}")
            return None

    def _check_current_url() -> dict | None:
        url = str(page.url or "")
        captured = str(getattr(page, "_omp_last_oauth_callback_url", "") or "")
        if captured:
            result = _try_extract_callback(captured)
            if result:
                return result
    ...
    def _wait_for_callback(timeout_sec: int) -> dict | None:
        ...
        while time.time() < deadline:
            ...
            if url and url not in checked_urls:
                ...
                if _is_oauth_callback_url(url) or "localhost" in url:
            ...
            captured = str(getattr(page, "_omp_last_oauth_callback_url", "") or "")
            for candidate in (url, captured):
                if _is_oauth_callback_url(candidate):
                    result = _try_extract_callback(candidate)
                    if result:
                        return result
            time.sleep(0.8)
        # 最后再检查一次
        try:
            ...
            captured = str(getattr(page, "_omp_last_oauth_callback_url", "") or "")
            for candidate in (final_url, captured):
                if _is_oauth_callback_url(candidate):
                    log(f"  [callback_wait] 超时后最终 callback: {candidate[:150]}")
                    result = _try_extract_callback(candidate)
                    if result:
                        return result
        except Exception:
...
def _submit_oauth_password_direct(page, password: str, log) -> dict:
    ...
    while time.time() < deadline:
        ...
        state = _derive_registration_state_from_page(page)
        page_type = str(state.get("page_type") or "")
        if page_type in {"email_otp_verification", "about_you", "add_email", "consent", "workspace_selection",
...
def _submit_password_via_page(page, password: str, log) -> dict:
    ...
    while time.time() < deadline:
        try:
            if page.is_closed():
                return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "密码页提交后页面关闭，疑似代理或 OpenAI 前端中断"}
        except Exception:
            return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "密码页提交后页面关闭，疑似代理或 OpenAI 前端中断"}
        current_url = str(page.url or "")
        last_url = current_url or last_url
        ...
        token_check = ""
        try:
            token_check = str(page.evaluate("() => localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken') || ''") or "")
        except Exception:
            pass
        if token_check and len(token_check) > 20:
            log(f"密码页提交后检测到 access token，按成功处理")
            return {"ok": True, "status": 200, "url": current_url, "data": None, "text": ""}
    ...
    _dump_debug(page, "chatgpt_password_fail")
    try:
        final_state = _derive_registration_state_from_page(page)
        final_page_type = str(final_state.get("page_type") or "-")
    except Exception:
        final_page_type = "-"
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": f"密码页提交后未跳转: page={final_page_type} url={last_url[:120]}"}


def _submit_otp_via_page(page, code: str, log, device_id: str = "", user_agent: str = "") -> dict:
    otp = str(code or "").strip()
    if not otp:
    ...
    while time.time() < deadline:
        ...
        if any(fragment in lowered_body for fragment in [
            "invalid code",
            "wrong code",
            "incorrect code",
            "wrong_email_otp_code",
            ...
            return {"ok": False, "status": 400, "url": current_url, "data": None, "text": body_text[:300]}
        time.sleep(0.5)
    if device_id:
        validator = _validate_browser_phone_otp if "phone" in str(last_url or page.url).lower() else _validate_browser_email_otp
        api_resp = validator(page, otp, device_id, user_agent or _random_chrome_ua(), last_url)
        api_status = int(api_resp.get("status") or 0)
        log(f"验证码页 API fallback: status={api_status} url={api_resp.get('url') or ''}")
        if api_resp.get("ok") or api_status in (200, 201, 204):
            return {"ok": True, "status": api_status or 200, "url": api_resp.get("url") or page.url, "data": api_resp.get("data"), "text": ""}
        text = str(api_resp.get("text") or "")[:300]
        if text:
            return {"ok": False, "status": api_status, "url": api_resp.get("url") or last_url, "data": api_resp.get("data"), "text": text}
...
def _submit_about_you_via_page(page, log) -> dict:
    ...
    def _remember_callback_url(value: str) -> None:
        text = str(value or "")
        if "code=" in text and text not in callback_urls:
            callback_urls.append(text)

    try:
        page.on("request", lambda req: _remember_callback_url(getattr(req, "url", "")))
        page.on("response", lambda resp: _remember_callback_url(getattr(resp, "url", "")))
    except Exception:
        pass
    ...
    excluded_name_indices = {int(name_entry.get("visibleIndex"))} if name_entry and str(name_entry.get("visibleIndex", "")).isdigit() else set()
    age_entry = _pick_best_about_you_input(
        visible_inputs,
        "age",
        exclude_visible_indices=excluded_name_indices,
    )
    birth_year_entry = _pick_best_about_you_input(
        visible_inputs,
        "birth_year",
        exclude_visible_indices=excluded_name_indices,
    )
    birth_year_month_entry = _pick_best_about_you_input(
        visible_inputs,
        "birth_year_month",
        exclude_visible_indices=excluded_name_indices,
    )
    ...
    name_candidates = [
        page.get_by_label(re.compile(r"full\s*name|氏名", re.IGNORECASE)),
        page.get_by_label(re.compile(r"全名|姓名", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"full\s*name|name|氏名", re.IGNORECASE)),
        ...
        page.locator("xpath=//*[contains(normalize-space(string(.)),'全名') or contains(normalize-space(string(.)),'姓名') or contains(normalize-space(string(.)),'氏名')]/following::input[1]"),
    ]
    birthday_candidates = [
        page.get_by_label(re.compile(r"birthday|date of birth|birth|生年月日|誕生日", re.IGNORECASE)),
        page.get_by_label(re.compile(r"生日|出生|生年月日|誕生日", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"birthday|date of birth|birth|生年月日|誕生日", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"生日|出生|生年月日|誕生日", re.IGNORECASE)),
        page.get_by_placeholder(re.compile(r"mm.?dd.?yyyy|yyyy.?mm.?dd|yyyy/mm/dd|birthday|生日|生年月日|年/?月/?日", re.IGNORECASE)),
        ...
        page.locator("xpath=//*[contains(normalize-space(string(.)),'生日') or contains(normalize-space(string(.)),'出生') or contains(normalize-space(string(.)),'生年月日') or contains(normalize-space(string(.)),'誕生日')]/following::input[1]"),
        page.locator("input[type='date']"),
    ]
    birth_year_candidates = [
        page.get_by_label(re.compile(r"birth\s*year|year\s*of\s*birth|^生年$|生年", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"birth\s*year|year\s*of\s*birth|^生年$|生年", re.IGNORECASE)),
        page.get_by_placeholder(re.compile(r"yyyy", re.IGNORECASE)),
        page.locator("xpath=//*[normalize-space(string(.))='生年' or contains(translate(normalize-space(string(.)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'birth year') or contains(translate(normalize-space(string(.)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'year of birth')]/following::input[1]"),
    ]
    birth_year_month_candidates = [
        page.get_by_label(re.compile(r"birth\s*year\s*month|year\s*and\s*month\s*of\s*birth|birth\s*month|^生年月$|生年月", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"birth\s*year\s*month|year\s*and\s*month\s*of\s*birth|birth\s*month|^生年月$|生年月", re.IGNORECASE)),
        page.get_by_placeholder(re.compile(r"yyyy.?mm|yyyy/mm|yyyy-mm", re.IGNORECASE)),
        page.locator("xpath=//*[normalize-space(string(.))='生年月' or contains(translate(normalize-space(string(.)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'birth month') or contains(translate(normalize-space(string(.)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'year and month of birth')]/following::input[1]"),
    ]
    ...
    age_candidates = [
        page.get_by_label(re.compile(r"age|年齢|何才|何歳", re.IGNORECASE)),
        page.get_by_label(re.compile(r"年龄", re.IGNORECASE)),
        page.get_by_role("textbox", name=re.compile(r"age|年齢|何才|何歳", re.IGNORECASE)),
        ...
        page.locator(
            "xpath=//*[contains(translate(normalize-space(string(.)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'age') or contains(normalize-space(string(.)),'年齢') or contains(normalize-space(string(.)),'何才') or contains(normalize-space(string(.)),'何歳')]/following::input[1]"
        ),
    ...
    try:
        mode_probe = page.evaluate(
            ...
            () => {
              ...
              const hasAge = allText.some((t) => t === 'age' || t === 'edad' || t === 'âge' || t === 'alter' || t === 'idade' || t.includes('how old') || t.includes('年龄') || t.includes('年齢') || t.includes('何才') || t.includes('何歳') || t.includes('나이'));
              const hasBirthYearMonth = allText.some((t) => t.includes('birth month') || t.includes('birth year month') || t.includes('year and month of birth') || t === '生年月' || (t.includes('生年月') && !t.includes('生年月日')));
              const hasBirthYear = allText.some((t) => t.includes('birth year') || t.includes('year of birth') || t === '生年' || (t.includes('生年') && !t.includes('生年月')));
              ...
              return { labels, placeholders, headings, hasAge, hasBirthday, hasBirthYear, hasBirthYearMonth };
            }
            """
        ) or {}
    except Exception:
    ...
    has_birth_year_month_label = bool(mode_probe.get("hasBirthYearMonth"))
    ...
    has_birth_year_field = any(_has_visible(candidate) for candidate in birth_year_candidates[:2]) or birth_year_entry is not None
    has_birth_year_month_field = any(_has_visible(candidate) for candidate in birth_year_month_candidates[:2]) or birth_year_month_entry is not None
    has_birthday_select = False
    try:
        has_birthday_select = page.locator("select:visible").count() >= 2
    except Exception:
        has_birthday_select = False
    about_mode = _select_about_you_mode(
        has_age_label=has_age_label,
        has_birthday_label=has_birthday_label,
        has_birth_year_label=has_birth_year_label,
        has_birth_year_month_label=has_birth_year_month_label,
        has_age_field=has_age_field,
        has_birthday_field=has_birthday_field,
        has_birth_year_field=has_birth_year_field,
        has_birth_year_month_field=has_birth_year_month_field,
    ...
    if about_mode == "birthday_select":
    ...
    elif about_mode == "birth_year_month":
        if len(date_parts) == 3:
            year_month_values = [f"{yyyy}/{mm}", f"{yyyy}-{mm}", f"{yyyy}{mm}"]
            if _fill_visible_input_entry(birth_year_month_entry, year_month_values[0]):
                fill_result["month"] = True
                fill_result["year"] = True
                fill_result["birthdate"] = True
            if not fill_result.get("birthdate"):
                for candidate in birth_year_month_candidates:
            ...
            if not fill_result.get("birthdate"):
                ...
                if _fill_second_visible_input(year_month_values, excluded_visible_indices=excluded_indices):
                    fill_result["month"] = True
                    fill_result["year"] = True
                    fill_result["birthdate"] = True
        ...
        if len(date_parts) == 3:
            if _fill_visible_input_entry(birth_year_entry, yyyy):
                fill_result["year"] = True
                fill_result["birthdate"] = True
            if not fill_result.get("year"):
                for candidate in birth_year_candidates:
                    if _fill_locator(candidate, yyyy):
                        fill_result["year"] = True
                        fill_result["birthdate"] = True
                        break
            if not fill_result.get("year"):
                ...
                if name_entry and str(name_entry.get("visibleIndex", "")).isdigit():
                    excluded_indices.add(int(name_entry.get("visibleIndex")))
                if _fill_second_visible_input([yyyy], excluded_visible_indices=excluded_indices):
                    fill_result["year"] = True
                    fill_result["birthdate"] = True
        if len(date_parts) == 3 and _sync_hidden_birthday_input(page, f"{yyyy}-{mm}-{dd}", log):
            fill_result["birthdate"] = True
    ...
    for attempt in range(3):
        if attempt:
            time.sleep(0.8)
        try:
            current_url_before_click = str(page.url or "")
        except Exception:
            current_url_before_click = ""
        if current_url_before_click and "about-you" not in current_url_before_click:
            submit_selector = f"verified-before-click:{current_url_before_click[:80]}"
            break
        try:
            page.evaluate("() => { document.activeElement?.blur?.(); }")
        except Exception:
        ...
        for sel in SUBMIT_SELECTORS[:4]:
            try:
                ...
                if box:
                    ...
                    time.sleep(0.08)
                    page.mouse.up()
                    clicked_label = f"mouse-center:{sel}"
                else:
                    el.click(timeout=3000)
                    clicked_label = f"normal:{sel}"
                log(f"about_you 已点击继续按钮: {clicked_label}")
                break
            except Exception as exc:
                try:
                    current_url_after_error = str(page.url or "")
                except Exception:
                    current_url_after_error = ""
                if current_url_after_error and "about-you" not in current_url_after_error:
                    submit_selector = f"verified-after-selector-error:{current_url_after_error[:80]}"
                    break
                last_click_error = str(exc).splitlines()[0][:180]
                continue
        if submit_selector:
            break
        ...
        while time.time() < verify_deadline:
            try:
                if page.is_closed():
                    return {"ok": False, "status": 0, "url": before_url, "data": None, "text": "about_you 点击后页面关闭，未验证成功，不按成功处理"}
            except Exception:
                return {"ok": False, "status": 0, "url": before_url, "data": None, "text": "about_you 点击后页面关闭，未验证成功，不按成功处理"}
            try:
                current_url = str(page.url or "")
            ...
            if current_url and current_url != before_url and "about-you" not in current_url:
                submit_selector = clicked_label
                break
            try:
                token_check = str(page.evaluate("() => localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken') || ''") or "")
            except Exception:
        ...
        if error_text:
            ...
            return {"ok": False, "status": 400, "url": before_url, "data": None, "text": f"about_you 服务端拒绝或字段错误: {error_text}"}
        log(f"about_you 点击未生效，继续重试: attempt={attempt + 1}, before={before_url[:80]}")
    if not submit_selector:
        error_text = _extract_auth_error_text(page)
        detail = error_text or last_click_error or "未离开 about-you 页面"
        _dump_debug(page, "chatgpt_about_you_fail")
        return {"ok": False, "status": 400 if error_text else 0, "url": str(page.url or ""), "data": None, "text": f"about_you 未成功提交: {detail}"}
    ...
    if not last_url:
        time.sleep(1)
        return {"ok": False, "status": 0, "url": "", "data": None, "text": "about_you 点击后页面关闭，未验证成功"}
    while time.time() < deadline:
        try:
            if page.is_closed():
                return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "about_you 点击后页面关闭，未验证成功"}
        except Exception:
            return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "about_you 点击后页面关闭，未验证成功"}
        try:
            current_url = page.url
            ...
            if "closed" in str(exc).lower():
                return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "about_you 点击后页面关闭，未验证成功"}
            raise
        if callback_urls and "chatgpt.com" in current_url and "/auth/error" not in current_url:
            return {"ok": True, "status": 200, "url": current_url, "data": None, "text": ""}
                ...
                try:
                    js_clicked = page.evaluate(
                        ...
                        () => {
                          ...
                          const target = nodes.find((el) => {
                            const text = String(el.innerText || el.textContent || '').replace(/\\s+/g, ' ').trim();
                            return visible(el) && /アカウントの作成を完了|finish creating account/i.test(text);
                          });
                          if (!target) return false;
                    ...
                    if js_clicked:
                        log("about_you 重试 JS dispatchEvent: アカウントの作成を完了する")
                        time.sleep(0.5)
                        continue
                except Exception:
                    pass
        ...
        time.sleep(0.5)
    if callback_urls:
        return {"ok": True, "status": 200, "url": callback_urls[-1], "data": None, "text": ""}
    try:
        _dump_debug(page, "chatgpt_about_you_fail")
    except Exception:
        pass
...
def _browser_registration_flow(page, email: str, password: str, otp_callback, phone_callback, log) -> dict:
    ...
    for step in range(12):
        ...
        if _is_registration_complete(state):
            _complete_oauth_callback_navigation(page, state, log)
        ...
        if _is_email_otp(state):
            ...
            otp_resp = _submit_otp_via_page(page, code, log, device_id=device_id, user_agent=user_agent)
...
class ChatGPTBrowserRegister:
    ...
    def run(self, email: str, password: str) -> dict:
        ...
        with _open_camoufox_browser(launch_opts) as browser:
